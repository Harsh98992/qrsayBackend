...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  const restaurantData = await Restaurant.findOne({ _id: user.restaurantKey });
  ...
  try {
    const resetURL = `${req.get("origin")}/admin/resetPassword/${resetToken}`;
    ...
    if (restaurantData?.restaurantEmail) {
      await sendEmail(restaurantData?.restaurantEmail, "Password reset", emailText);
    }
