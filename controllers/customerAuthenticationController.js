...
function isOtpExpired(givenDate) {
  ...
  // console.log(otpCreatedAt,elapsedTimeInSeconds);
  // return elapsedTimeInSeconds > expirationTimeInSeconds;

  var currentDate = new Date();

  // Calculate the given date with an additional 10 minutes
  var givenDatePlus10Minutes = new Date(givenDate);
  givenDatePlus10Minutes.setMinutes(givenDatePlus10Minutes.getMinutes() + 10);
    ...
    res.status(200).json({
      ...
      data: {
        message: `The One-Time Password (OTP) has been sent to ${phoneNumber}. Please check your ${
          socialLogin === "sms" ? "SMS messages." : "WhatsApp messages."
        }`,
      },
    });
  } catch (error) {
...
exports.whatsappLogin = catchAsync(async (req, res, next) => {
  ...
  if (!phoneNumber) {
    return next(
      new AppError("Phone Number is required to verify your account.", 400)
    );
  }
