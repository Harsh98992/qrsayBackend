...
const unlockTable = async (orderData, completeflag = true, req, res, next) => {
  const queryResult = await Table.findOne({
    restaurantId: orderData.restaurantId,
    tables: { $elemMatch: { key: "customerId", value: orderData.customerId } },
  });
...
exports.changeOrderStatus = catchAsync(async (req, res, next) => {
  ...
  if (req.body.orderStatus === "rejected") {
    ...
    // send a mail to the customer that order has been accepted by the restaurant
    sendMail(
      orderData.customerEmail,
    ...
    // send a mail to the customer that order has been completed successfully
    sendMail(
      orderData.customerEmail,
